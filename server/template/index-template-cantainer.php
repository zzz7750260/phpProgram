					...
					<ul class="list-group">
						<?php 
							...
							foreach($cinemaArticleListArray as $key => $value){
								$cinemaArticleListHtml = '<li class="list-group-item"><a href="'.$commonUtil->isHttpsCheckSelect().'//'.$_SERVER['HTTP_HOST'].'/article/'.$value['categoryyw'].'/show-'.$value['aid'].'.html">'.$value['title'].'</a></li>';
